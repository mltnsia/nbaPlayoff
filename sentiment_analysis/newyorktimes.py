...
team_list = ["celtics", "nets", "knicks", "76ers", "raptors",
...
def main():
    ...
    # fill in teams
    year_list = final_df['season'].unique()
    combinations = list(itertools.product(year_list, team_list))
    df = pd.DataFrame(combinations, columns=['season', 'team'])
    merged = pd.merge(final_df, df, on=['season', 'team'], how='right')
    merged['score'] = merged['score'].fillna(0)
    team_id = pd.read_csv("id_name_mapping.csv")
    nyt_scores = mapID(team_id, merged)
    nyt_scores.to_csv('nyt_scores.csv', index=False)

# main()

def mapID(team_id, merged):
    team_id['TeamName'] = team_id['TeamName'].str.lower()
    mapped_teams = pd.merge(team_id, merged, left_on="TeamName", right_on="team", how="inner")
    mapped_teams = mapped_teams.drop('team', axis=1)
    mapped_teams= mapped_teams.sort_values('season')
    return mapped_teams
